...
impl Component for Model {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        html! {
            <>
                { for self.exprs.iter().enumerate().map(|expr| self.view_expr(ctx, expr)) }
...
impl Model {
    ...
    fn view_expr(&self, ctx: &Context<Self>, (i, expr): (usize, &String)) -> Html {
        ...
        html! {
            <div>
                <input type="text" inputmode="decimal" value={expr.to_string()} oninput={ctx.link().callback(move |e: InputEvent| Msg::GotInput(i, e.target().unwrap().dyn_into::<HtmlInputElement>().unwrap().value()))}/>
                {content}
            </div>
        }
    }
